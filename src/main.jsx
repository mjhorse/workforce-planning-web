...
function createEmptyDemand() {
  return {
    ...
    rr: '', ir: '', priority: '高', version: 'V2026.08', landingDate: '2026-08-31', poPeriod: getCurrentMonthKey(), days: 30,
...
function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {page === 'personnel' && <Personnel personnel={personnel} setPersonnel={setPersonnel} demands={demands} budget={budget} openDemand={openDemand} openDemandIds={openDemandIds}/>}
...
const demandColumnLabels = {
  title: '需求', demandNo: '需求单号', requester: '提出人', demandBA: '需求BA', demandPM: '需求PM', source: '类型', acceptanceCriteria: '验收标准', investment: '需求方', priority: '优先级', landingDate: '计划落地日期', poPeriod: 'PO执行期间', budgetStatus: '预算状态', budgetSourceType: '预算来源', budgetSource: '其他预算来源', budgetAcquiredDate: '预算获取日期', budgetContact: '预算接口人', budgetEta: '预算承诺日期', days: '预估人天', finalDays: '决算人力', version: '版本', status: '需求状态', settlementDays: '结算人力'
};
...
function DemandPool({demands, setDemands, budget, personnel = [], totalDays, unfundedDays}) {
  ...
  const defaultColumnKeys = ['title','demandNo','requester','source','investment','priority','landingDate','poPeriod','budgetStatus','budgetSourceType','budgetSource','budgetAcquiredDate','budgetContact','budgetEta','days','finalDays','version','status','settlementDays'];
  ...
  const fullTextFields = ['title','demandNo','source','acceptanceCriteria','investment','budgetSourceType','budgetSource','requester','version','landingDate','poPeriod','budgetContact','budgetEta','budgetStatus','rr','ir','priority','review','status'];
...
const personnelImportFields = [
  ['name', '人员姓名', ['人员姓名', '姓名', '人员']], ['employeeNo', '工号', ['工号', '员工号', '人员编号']], ['owner', '团队/负责人', ['团队/负责人', '团队', '负责人', '归属负责人', '团队负责人']], ['position', '岗位', ['岗位', '职位', '角色']], ['location', '地点', ['地点', '办公地点', '城市', '所在地']], ['supplier', '供应商', ['供应商', '厂商']], ['entryDate', '入项时间', ['入项时间', '入场时间', '入项日期']], ['dailyCost', '人天成本（元/人天）', ['人天成本（元/人天）', '人天成本', '元/人天']], ['monthlyDays', '月度可用人天', ['月度可用人天', '月可用人天']]
];
...
function Personnel({personnel, setPersonnel, demands = [], budget, openDemand, openDemandIds}) {
  const [importMsg, setImportMsg] = useState('');
  const [importDraft, setImportDraft] = useState(null);
  const [addDraft, setAddDraft] = useState(null);
  const [personnelSort, setPersonnelSort] = useState({ key: '', direction: 'asc' });
  const [activeTab, setActiveTab] = useState('team');
  const [poPeriod, setPoPeriod] = useState(() => getCurrentMonthKey());
  ...
  const update=(id,k,v)=>{ if (k === 'employeeNo') { const employeeNo = String(v).trim(); if (employeeNo && normalized.some(p => p.id !== id && String(p.employeeNo).trim() === employeeNo)) { alert(`工号已存在：${employeeNo}`); return; } } setPersonnel(ps=>normalized.map(p=>p.id===id?{...p,[k]:v}:p)); };
  const openAddPersonnel = () => setAddDraft({ name: '', employeeNo: '' });
  const confirmAddPersonnel=()=>{ const name = addDraft.name.trim(); const employeeNo = addDraft.employeeNo.trim(); if (!name) { alert('新增人员失败：请填写人员姓名'); return; } if (!employeeNo) { alert('新增人员失败：请填写工号'); return; } if (normalized.some(p => String(p.employeeNo).trim() === employeeNo)) { alert(`新增人员失败：工号已存在：${employeeNo}`); return; } setPersonnel(ps=>[...normalized,{id:Date.now(),name,employeeNo,owner:'',position:'后端',location:'',supplier:'',entryDate:'',dailyCost:0,monthlyDays:22.5}]); setAddDraft(null); };
  ...
  const confirmImport = () => { try { const imported = parsePersonnelCsv(importDraft.text, importDraft.mapping); if (!imported.length) throw new Error('没有可导入的人员数据'); setPersonnel(ps => [...normalized, ...imported]); setImportMsg(`已按字段映射导入 ${imported.length} 条人员管理`); setImportDraft(null); } catch (err) { setImportMsg(`导入失败：${err.message}`); } };
  const poOptions = useMemo(() => buildPoPeriodOptions(demands), [demands]);
  React.useEffect(() => { if (!poOptions.includes(poPeriod)) setPoPeriod(poOptions[0] || getCurrentMonthKey()); }, [poOptions, poPeriod]);
...
function downloadTemplate() {
  const headers = ['需求名称','需求单号','类型','需求方','预算来源','其他预算来源','预算池ID','需求BA','需求PM','当前业务痛点','需求目标','需求价值','验收标准','需求提出人','业务评审结论','需求状态','IR单号','优先级','版本','计划落地日期','PO执行期间','工作量填写方式','总工作量（人天）预估','需求分析工作量预估','前端开发工作量预估','中台开发工作量预估','后台开发工作量预估','决算人力','结算人力','是否带预算','历史/参考预算金额w','预算接口人','预算承诺日期','预算获取日期','预算状态'];
  const sample = ['示例需求','RR2026053012345','产品立项规划',baselineDemandParty,'部门基线','','','李BA','王PM','当前流程效率低','优化处理流程','提升效率','上线后核心流程可验证且异常可回滚','张三','待评审','待评估','IR-001','高','','2026-08-31','2026-08','total','30','5','10','8','7','0','0','否','0','','','','已承诺未获取'];
...
function downloadCsv(demands) {
  const headers = ['需求名称','需求单号','类型','需求方','预算来源','其他预算来源','预算池ID','需求BA','需求PM','验收标准','IR单号','优先级','版本','计划落地日期','PO执行期间','工作量填写方式','总工作量','决算人力','结算人力','历史/参考预算金额w','预算接口人','预算承诺日期','预算获取日期','预算状态','评审结论','需求状态'];
  const lines = [headers.join(',')].concat(demands.map(d => [d.title,d.demandNo,d.source,d.investment,d.budgetSourceType || '部门基线',d.budgetSourceType === '其他' ? (d.budgetSource || '') : '',d.budgetPoolId || '',d.demandBA || '',d.demandPM || '',d.acceptanceCriteria || '',d.ir,d.priority,d.version,d.landingDate || '',getDemandPoPeriod(d),d.workloadMode || 'total',toNumber(d.days),toNumber(d.finalDays),toNumber(d.settlementDays),d.budget,d.budgetContact || '',d.budgetEta || '',d.budgetAcquiredDate || '',d.budgetStatus || '无预算',d.review,d.status].map(v=>`"${String(v).replaceAll('"','""')}"`).join(',')));
  const blob = new Blob([lines.join('\n')], {type:'text/csv;charset=utf-8'});
